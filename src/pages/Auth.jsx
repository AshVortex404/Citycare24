import { useState, useContext } from 'react';
import { useNavigate } from 'react-router-dom';
import { login, register } from '../utils/api';
import { AuthContext } from '../utils/AuthContext';
import './Auth.css';

const Auth = () => {
    const [isLogin, setIsLogin] = useState(true);
    const [loginMode, setLoginMode] = useState('user'); // 'user' or 'admin'
    const [username, setUsername] = useState('');
    const [password, setPassword] = useState('');
    const [error, setError] = useState('');
    const [loading, setLoading] = useState(false);

    const { loginUser } = useContext(AuthContext);
    const navigate = useNavigate();

    const handleSubmit = async (e) => {
        e.preventDefault();
        setError('');
        setLoading(true);

        try {
            if (isLogin) {
                const { data } = await login(username, password);

                // Verify role matches selected login mode
                if (loginMode === 'admin' && data.role !== 'admin') {
                    setError('Invalid admin credentials. Please use admin login.');
                    setLoading(false);
                    return;
                }

                loginUser(data.token, data.role, data.userId);
                navigate('/');
            } else {
                await register(username, password);
                setIsLogin(true);
                setError('Registration successful! Please login.');
                setPassword('');
            }
        } catch (err) {
            setError(err.response?.data?.message || 'An error occurred');
        } finally {
            setLoading(false);
        }
    };

    const handleModeSwitch = (mode) => {
        setLoginMode(mode);
        setError('');
        setUsername('');
        setPassword('');
    };

    return (
        <div className="auth-container">
            <div className="auth-background">
                <div className="floating-circle circle-1"></div>
                <div className="floating-circle circle-2"></div>
                <div className="floating-circle circle-3"></div>
            </div>

            <div className="auth-card">
                <div className="auth-header">
                    <div className="logo-container">
                        <span className="logo-icon">🏛️</span>
                    </div>
                    <h1 className="fade-in">CivicWatch</h1>
                    <p className="fade-in-delay">Report and track civic issues in your community</p>
                </div>

                <div className="auth-tabs">
                    <button
                        className={isLogin ? 'active' : ''}
                        onClick={() => { setIsLogin(true); setError(''); }}
                    >
                        Login
                    </button>
                    <button
                        className={!isLogin ? 'active' : ''}
                        onClick={() => { setIsLogin(false); setError(''); setLoginMode('user'); }}
                    >
                        Register
                    </button>
                </div>

                {isLogin && (
                    <div className="login-mode-selector">
                        <button
                            type="button"
                            className={`mode-btn ${loginMode === 'user' ? 'active' : ''}`}
                            onClick={() => handleModeSwitch('user')}
                        >
                            <img
                                src="data:image/png;base64,"
                                alt="User"
                                className="mode-icon"
                            />
                            <span className="mode-text">User Login</span>
                        </button>
                        <button
                            type="button"
                            className={`mode-btn ${loginMode === 'admin' ? 'active' : ''}`}
                            onClick={() => handleModeSwitch('admin')}
                        >
                            <img
                                src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTlHWbevVnhD_QGLdNmXmZj5yfKKxk432eLmA&s"
                                alt="Admin"
                                className="mode-icon"
                            />
                            <span className="mode-text">Admin Login</span>
                        </button>
                    </div>
                )}

                <form onSubmit={handleSubmit} className="auth-form">
                    <div className="form-group">
                        <label>
                            <span className="label-icon">👤</span>
                            Username
                        </label>
                        <input
                            type="text"
                            value={username}
                            onChange={(e) => setUsername(e.target.value)}
                            required
                            placeholder={isLogin && loginMode === 'admin' ? 'Enter admin username' : 'Enter your username'}
                            className="animated-input"
                        />
                    </div>

                    <div className="form-group">
                        <label>
                            <span className="label-icon">🔒</span>
                            Password
                        </label>
                        <input
                            type="password"
                            value={password}
                            onChange={(e) => setPassword(e.target.value)}
                            required
                            placeholder={isLogin && loginMode === 'admin' ? 'Enter admin password' : 'Enter your password'}
                            className="animated-input"
                        />
                    </div>

                    {error && (
                        <div className={`error-message ${error.includes('successful') ? 'success' : ''} slide-in`}>
                            {error}
                        </div>
                    )}

                    <button type="submit" className="submit-btn pulse-btn" disabled={loading}>
                        {loading ? (
                            <>
                                <span className="spinner-small"></span>
                                Please wait...
                            </>
                        ) : (
                            <>
                                {isLogin ? (
                                    loginMode === 'admin' ? '👑 Login as Admin' : '🚀 Login'
                                ) : '✨ Create Account'}
                            </>
                        )}
                    </button>
                </form>

                {isLogin && loginMode === 'admin' && (
                    <div className="admin-hint">
                        <p>💡 Default admin credentials: admin / admin123</p>
                    </div>
                )}
            </div>
        </div>
    );
};

export default Auth;
